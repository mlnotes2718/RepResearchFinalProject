...
evtypeLst
```
## Strategy on Clean Up EVTYPE Mis-Classification
As shown above, the unique storm event classification has 440 unique classification, whereas in NOAA official storm classification, we have only 48 classes. Massive clean up is required. We have the following methods to re-classified.

*Method 1*
The most ideal re-classification method is to read the remarks column and determine the actual nature of storm type. However, this is a extremely tediuos task. It might not be effective when our analysis try to account for majore casualties and economic impact.

*Method 2*
The second method is to re-classified the storm event manually on the obvious error such as spelling error.
