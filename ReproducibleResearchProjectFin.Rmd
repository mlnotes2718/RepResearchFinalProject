...
3.  File Structure after reduction.
...
Clean up EVTYPE Beside remove leading, trailing and double space, we also convert all storm type to upper case.
...
We also need to convert the exponent of the property and crop damage. The exponent of k or K represent thousands. M or m in millions. The number is 10 to the power of the number. (e.g. 7 = 10^7)

First we need to find out the unique exponent of PROPDMGEXP and CROPDMGEXP.

```{r}
uniquePROPDMGEXP <- toupper(cleanStormData$PROPDMGEXP)
uniquePROPDMGEXP <- unique(uniquePROPDMGEXP)
uniqueCROPDMGEXP <- toupper(cleanStormData$CROPDMGEXP)
uniqueCROPDMGEXP <- unique(uniqueCROPDMGEXP)
...
```{r}
replacement <- c(0,0,0,1,10^0,10^2,10^3,10^4,10^5,10^6,10^7,1000000000,100,1000,1000000)
refTable <- cbind(uniqueBothDMGEXP, replacement)
#cleanStormData$PROPDMGEXP <- replace(cleanStormData$PROPDMGEXP, cleanStormData$PROPDMGEXP %in% uniqueBothDMGEXP, replacement)
cleanStormData$PROPDMGEXP[cleanStormData$PROPDMGEXP %in% uniqueBothDMGEXP] <- refTable["-"
                                                                              ]
```

2.  We then create a data frame that is group by the event type, removing fatalities and injuries that are 0.
